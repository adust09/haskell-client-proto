...
proposeBlock env state _store slot = do
  ...
  -- Attestation inclusion is handled by the aggregator in the new leanSpec
  -- model; propose an empty body for now.
  let body = BeaconBlockBody { bbbAttestations = forceRight $ mkSszList @128 [] }
